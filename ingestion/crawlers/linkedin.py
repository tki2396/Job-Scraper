...
from ingestion.crawlers.base import BaseAbstractCrawler
...
class LinkedinCrawler(BaseAbstractCrawler):
    ...
    def extract(self, link: str, **kwargs):
        print(f"Extracting LinkedIn profile: {link}")
        self.login()
        ...
        #     [
        #         PostDocument(
        #             platform="linkedin", content=post, author=kwargs.get("username")
        #         )
        #         for post in posts
        #     ]
        # )
        ...
        return super().extract(link, **kwargs)
    ...
    def _extract_image_urls(self, buttons: List[Tag]) -> Dict[str, str]:
        """Extract image URLs from the LinkedIn profile."""
        post_images = {}
    ...
    def _get_page_content(self, url: str) -> BeautifulSoup:
        """Get the page content of the LinkedIn profile."""
        self.driver.get(url)
        time.sleep(5)
    ...
    def _scrape_education(self, profile_url: str) -> str:
        ...
        time.sleep(5)
        soup = BeautifulSoup(self.driver.page_source, "html.parser")
        content = soup.find("section", {"id": "education-section"})
        return content.get_text(strip=True) if content else ""
    ...
    def login(self):
        ...
        if not settings.LINKEDIN_USERNAME or not settings.LINKEDIN_PASSWORD:
            raise ValueError("LinkedIn username and password not set.")
